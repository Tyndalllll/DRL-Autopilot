...
class SAC_Agent(object):
	...
	def save(self):
		torch.save(self.actor.state_dict(), "./model/sac_actor_test.pth")
		torch.save(self.q_critic.state_dict(), "./model/sac_q_critic_test.pth")


	def load(self):
		self.actor.load_state_dict(torch.load("./model/sac_actor_test.pth"),False)
		self.q_critic.load_state_dict(torch.load("./model/sac_actor_test.pth"),False)
...
def plot_state(states,tag = 'test'):
    plt.figure()  # 创建一个图形实例，方便同时多画几个图
    plt.xlabel('epsiodes')
    plt.axis([-1, 1, -1, 1])
    states = np.array(states)
    colors = np.array(range(len(states[:,0])))
    plt.scatter(states[:,0],states[:,1], label='states',c= colors,s = 1 ,cmap= 'viridis')
    plt.legend()
    plt.colorbar()
    plt.show()
