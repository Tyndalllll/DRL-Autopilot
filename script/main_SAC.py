...
from SAC import SAC
from SAC import ReplayBuffer as ReplayBuffer

from env_im import Env
...
def main():
    ...
    for train_count in range(1):
        ...
        replay_buffer = ReplayBuffer(s_dim,a_dim)

        # replay_buffer_offline = ReplayBuffer_offline(args.demo_memory_size)

        agent = SAC(s_dim, a_dim, a_bound,args.batch_size)
            ...
            while not rospy.is_shutdown():
                ...
                for episode in range(args.episodes):
                    ...
                    for t in range(max_steps):
                        ...
                        # print(state)
                        if replay_buffer.count < 512:  # Take the random actions in the beginning for the better exploration
                        ...
                        replay_buffer.store(state, action, reward, observation, done)
                        # if t+1 % 10 == 0:
                        ...
                        if (done or (t == max_steps - 1)or env.region_out):
                            ...
                            print('\n\n')
                            steps = t
                            print(f"done: {done}, t: {t}, env.region_out: {env.region_out}")
